...
4. 直接双击 `start-obs-vcam.bat`。工具会自动读取已保存的 OBS 分辨率/FPS、检测端口和 DirectShow 活动模式；OBS 未运行时使用官方 `--startvirtualcam` 参数启动并实际读取一帧后才建立 MJPEG 服务。`--check` 只是可选的只读诊断。窗口必须保持打开，按 `Ctrl+C` 可停止服务。
...
方向只在一端配置：如果已经在 OBS 中旋转正确，手机“输入画面旋转”保持“不旋转”；如果 OBS 保持原始方向，则在手机端选择顺时针或逆时针 90°。手机端把网络帧放入独立的最新帧队列，由 Core Image GPU 变换一次后供照片、录像、预览和第三方输出复用；突发时自动合并过期中间帧，优先保持实时。
...
4. OBS 输出建议：H.264、目标分辨率与源 FPS、关键帧间隔 1 秒、无 B 帧或低延迟 preset；手机 HLS 解码目标可设为 1–240 FPS，实际可取得的帧率取决于 HLS 源和设备硬件。
5. 手机设置 URL：
...
`2160p` 只用于实验：手机设置中的“MJPEG 解码上限”也必须设为 3840，且 A10 上 4K MJPEG 30 FPS 可能出现高温、丢帧或内存压力。需要长期高质量运行时，优先使用 1080p MJPEG，或使用硬件解码的 HLS 工作流。
