# VirtualCamPro Windows 控制中心 2.11.0 部署教程
...
2.11.0 的 `standalone-self-test.bat` 不再只做文件哈希检查：它还会实际启动 GUI 冒烟测试、核心脚本 SelfTest、本机 HLS HTTP 服务，并验证 GET / HEAD / HTTP Range 206 / OPTIONS+CORS；如果检测到 FFmpeg，还会同时检查 MJPEG 与 libx264 编码器，并实际生成一小段 H.264 HLS 测试流。看到最终 `[OK]` 后再继续。如果完整性或运行级检查失败，优先处理第一条 `[FAIL]`；除 `obs-vcam-config.cmd` 外，不要单独修改交付文件。
...
3. 确认流 URL 正确，点击“检测当前网络流”。2.11.0 iOS 包同时包含 MJPEG 接收器、VideoToolbox/ImageIO 自动解码路径和基于 AVPlayer 的 HLS `.m3u8` 接收路径。
4. 彻底退出并重新打开相机或目标应用。
