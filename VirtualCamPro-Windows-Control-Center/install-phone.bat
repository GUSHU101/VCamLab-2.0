...
rem Load the shared stream configuration so --setup uses the same port/transport as the bridge.
...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass ^
    ...
    -Transport "%VCAM_INSTALL_TRANSPORT%"
exit /b %errorlevel%
...
echo   install-phone.bat --setup PHONE_IP [PACKAGE.deb] [SSH_PORT] [PHONE_FPS] [STREAM_URL] [mjpeg^|hls]
...
echo VCAM_PHONE_USER, VCAM_DEB_PATH, VCAM_PHONE_FPS, VCAM_STREAM_URL, VCAM_TRANSPORT.
exit /b 64
