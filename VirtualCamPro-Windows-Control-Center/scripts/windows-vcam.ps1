...
function Show-VcamEndpoints {
    ...
    if ($ListenerAddress -ne "0.0.0.0") {
        Write-Host "Listener URL: http://${ListenerAddress}:$ListenPort/live.mjpg"
    ...
    foreach ($entry in $addresses) {
        ...
        Write-Host ("  http://{0}:{1}/live.mjpg  [{2}]{3}" -f
            $entry.Address, $ListenPort, $entry.Interface, $suffix)
    }
}
...
function New-VcamFfmpegArguments {
    ...
    $filter = if ($exactObsRate -and $ObsMode.Width -eq $CanvasWidth -and
        ...
        # Preserve source pixels and timestamps when OBS already publishes the
        # exact requested mode. Avoiding no-op Lanczos and fps filters prevents
        # an unnecessary resample plus jitter-driven frame duplication/drops.
        "setsar=1"
    } else {
    ...
    $commonArguments = @(
        ...
        "-vf", $filter,
        "-c:v", "mjpeg",
        # The MJPEG encoder otherwise defaults to one worker on Windows.
        "-threads", [string]$MjpegEncoderThreads,
        "-pix_fmt", "yuvj420p",
        "-q:v", [string]$OutputQuality,
        "-huffman", "optimal",
        # Preserve exact OBS timestamps; file/fallback paths use the explicit
        # fps filter above when rate conversion is actually requested.
        "-fps_mode", "passthrough",
        # A bounded FIFO keeps a slow phone/TCP socket from back-pressuring
        # DirectShow. On overflow, discard network frames instead of blocking
        # OBS capture and filling its raw-frame buffer.
        "-f", "fifo",
        "-fifo_format", "mpjpeg",
        "-queue_size", [string]$NetworkQueueSize,
        "-drop_pkts_on_overflow", "1",
        "-format_opts", (("listen=1:flush_packets=1:send_buffer_size={0}:" +
            "tcp_nodelay=1:tcp_keepalive=1") -f ($NetworkSendBufferMB * 1MB)),
        $ListenerURL
    )
    $inputArguments = if ($InputSource -eq "obs") {
        ...
        # Use the requested profile rate for harmless DirectShow probe rounding,
        # but use the advertised rate when permissive mismatch mode selected a
        # genuinely different fallback mode.
    ...
    return @($inputArguments + $commonArguments)
}
...
"@
    ...
    if ($argumentText -like "*fps=*" -or
        ...
        $argumentText -notlike "*-format_opts`nlisten=1:flush_packets=1:send_buffer_size=4194304:tcp_nodelay=1:tcp_keepalive=1*" -or
...
Write-Host ((
    "Canvas: {0}x{1} at {2} FPS; MJPEG quality {3}; encoder threads {4}; scale mode {5}"
...
$outputURL = "http://{0}:{1}/live.mjpg" -f $effectiveBindAddress, $effectivePort
...
while ($true) {
    ...
    if (Test-VcamInterruptedExitCode -ExitCode $ffmpegExitCode) {
        Write-VcamStatus -Level "INFO" -Message "MJPEG bridge stopped by the user."
        exit 0
    }
    ...
    Write-VcamStatus -Level "WARN" -Message (
        "FFmpeg stopped with exit code {0}; restarting the listener in 1 second..." -f $ffmpegExitCode
    )
    Start-Sleep -Seconds 1
}
